const validate = (name, email, old_password, password, password_confirmation) => {
  ...
  password_confirmation.nextElementSibling.classList.remove('error_text_visible');

  let isValid = true;

  if (!name.value || name.value.length <= 3) {
    name.classList.add('error');
    name.nextElementSibling.innerText = "* O nome deve conter no mínimo 3 caracteres!"
